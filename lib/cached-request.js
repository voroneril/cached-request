...
function CachedRequest(request) {
   ...
   _request.autoDeleteJob = function () {
      self.autoDeleteJob();
   }
...
/**
 * Create a job to automatically delete a file by it's ttl
 * @param {*} options 
 */
CachedRequest.prototype.createJob = function (options) {
   queue.create('job-delete', options).delay(options.ttl).save();
};

/**
 * Init auto delete by queue
 */
CachedRequest.prototype.autoDeleteJob = function () {
   queue.process('job-delete', 10, function (job, done) {
      var file = job.data.filePath;
      var header = job.data.headerPath;
      ...
      fs.stat(header, function (err, stats) {
         if (err) {
            return console.error(err);
         }
         fs.unlink(header, function (err) {
            if (err) return console.log(err);
            console.log('file deleted successfully');
         });
      });
