...
function CachedRequest (request) {
  ...
  this.ttl = 0;
  this.age = 10800; // 3 hours
  this.cleanupInterval = 1000 * 60 * 30; //cleanup every 30 minutes
  ...
  _request.cleaUpCacheDirectory = function (options) {
    self.cleaUpCacheDirectory(options);
   }
...
/**
 * cleaUpCacheDirectory
 * Clean the cache directory base on it's given interval
 * @param {cleanupInterval, cacheDirectory, age} options 
 */
CachedRequest.prototype.cleaUpCacheDirectory = function (options) {
   const cleanupInterval = options.cleanupInterval || this.cleanupInterval;
   const cacheDirectory = options.cacheDirectory || this.cacheDirectory;
   const age = options.age || this.age;
   setInterval(function () {
      findRemoveSync(cacheDirectory, {
         files: "*.*",
         age: {
            seconds: age
         }
...
CachedRequest.prototype.parseHeaders = function (headers) {
  let last_index = headers.indexOf('}}');
  headers = headers.slice(0, last_index+2)
  let r = JSON.parse(headers).headers;
  return r;
};
...
CachedRequest.prototype.cachedRequest = function (...args) {
  ...
  const dataFilepath = `${this.cacheDirectory}${key}.gz`;
  const headersFilepath = `${this.cacheDirectory}${key}.meta.json`;
      ...
      responseReader.on("open", (error) => {
        ...
        headersReader.on("end", () => {
          try {
            ...
            console.log("ERROR parsing headers:", headersFilepath, response.headers);
            this.handleError(e);
            headersReader.close();
            return makeRequest();
          }
          ...
          stream.on('error', (error) => {
            ...
            console.log("ERROR with zlib!", 'dataFilepath', dataFilepath, 'url', options.url, 'headers', JSON.stringify(response.headers));
            this.handleError(error);
            headersReader.close();
            return makeRequest();
          });
          ...
          stream.on("end", () => {
            if (callback) {
              ...
              this.processBody(bodyBuffer, options, (err, body) => {
                ...
                callback(null, response, body, dataFilepath);
              });
            };
            response.end();
...
CachedRequest.prototype.makeRequest = function (options, ...args) {
  ...
  request.on("response", (response) => {
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      fs.writeFile(headersFilepath, JSON.stringify({options:args[0], headers:response.headers}), (error) => {
